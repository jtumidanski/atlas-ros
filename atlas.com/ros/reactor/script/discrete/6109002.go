...
func New6109002() script.Script {
	return generic.NewReactor(6109002, generic.SetAct(func(l logrus.FieldLogger, span opentracing.Span, db *gorm.DB, c script.Context) {
		...
		if c.MapId == 610030300 {
			generic.EventBlueMessage("MAGICIAN_SIGIL_ACTIVATED_LONG")
			next := event.GetProperty(l)(e.Id(), "glpq3") + 1
			event.SetProperty(l)(e.Id(), "glpq3", next)
			_map.MoveEnvironment(l)(c.WorldId, c.ChannelId, c.MapId, "menhir3", 1)
